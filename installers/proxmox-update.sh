...
print_header() {
  clear
  echo -e "${CYAN}╔══════════════════════════════════════╗${NC}"
  echo -e "${CYAN}║${WHITE}          MeTify LXC Updater          ${CYAN}║${NC}"
  echo -e "${CYAN}╚══════════════════════════════════════╝${NC}"
  echo
}

print_header
echo "Scanning for MeTify installations..."
echo "--------------------------------"
...
for id in "${ALL_CTIDS[@]}"; do
    ...
    if [[ "$status" == "running" ]]; then
        if pct exec "$id" -- test -d /opt/metify 2>/dev/null; then
            hostname=$(pct exec "$id" -- hostname 2>/dev/null || echo "?")
            FOUND_CTIDS+=("$id")
            echo -e "${GREEN}$id${NC}) $hostname ${GREEN}[MeTify found]${NC}"
        else
...
if [[ ${#RUNNING_UNCHECKED[@]} -gt 0 ]]; then
    echo -e "${YELLOW}Running, no MeTify found:${NC} ${RUNNING_UNCHECKED[*]}"
fi
...
BEFORE_HASH=$(pct exec "$CTID" -- bash -c "cd /opt/metify && git rev-parse HEAD")
...
AFTER_HASH=$(pct exec "$CTID" -- bash -c "cd /opt/metify && git rev-parse HEAD")
...
pct exec "$CTID" -- systemctl restart metify

sleep 3

if pct exec "$CTID" -- systemctl is-active --quiet metify; then
    echo -e "${GREEN}Service is running.${NC}"
else
    echo -e "${RED}Service failed to start. Check logs with:${NC}"
    echo -e "${WHITE}pct exec $CTID -- journalctl -u metify -n 50 --no-pager${NC}"
    exit 1
fi
